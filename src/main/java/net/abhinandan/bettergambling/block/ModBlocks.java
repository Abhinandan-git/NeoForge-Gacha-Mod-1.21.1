...
public class ModBlocks {
    ...
    public static final DeferredBlock<Block> WHEEL_BLOCK = registerBlock(
            "wheel_block", () -> new WheelBlock(
                    BlockBehaviour.Properties.of().strength(1f).noOcclusion()
            )
    );

    public static <T extends Block> @NotNull DeferredBlock<Block> registerBlock(String name, Supplier<T> block) {
        DeferredBlock<Block> toReturn = BLOCKS.register(name, block);
        registerItems(name, toReturn);
        return toReturn;
    }

    public static <T extends Block> void registerItems(String name, DeferredBlock<T> block) {
        ModItems.ITEMS.registerItem(
                name, properties -> new BlockItem(block.get(), new Item.Properties())
        );
    }
